...
public final class Scope {
  ...
  /**
   * Entry-point to the Scope processing engine. This is placed in the default package only to make
   * it simple to invoke from the command-line.
   *
   * <p>Executing the class takes two non-negative integers as command-line arguments. The second
   * argument defaults to 50 if it is omitted.
   *
   * <ul>
   *   <li>the maximum number of top candidate targets to list (0-2^31).
   *   <li>the minimal percentage confidence we would like for any listed candidate target (0-100).
   * </ul>
   *
   * The standard input this program expects is the contents of the scope, in the same format as a
   * <code>.blf</code> file.
   *
   * <p>It will produce a list of candidate targets, their locations and confidences in a
   * human-readable text format to its standard output.
   */
  public static void main(String[] args) {
    ...
    try {
      ...
      List<CandidateTarget> candidates =
          Analyzer.findCandidateTargets(scope, limitConfidence / 100.0f, limitCount);
  ...
  private static void usage() {
    ...
    System.err.println(
        "<confidence> is the percentage confidence (0-100) those target must exceed.");

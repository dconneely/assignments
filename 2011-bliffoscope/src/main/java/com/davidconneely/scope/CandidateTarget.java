...
/**
 * Represent the type and location of a candidate target in the scope
 * data, also contains an indication of our confidence in the target.
 */
public final class CandidateTarget {
	 ...
	 * 
	 * @param type
	 *            <code>0</code> for a slime torpedo; <code>1</code> for a
	 *            Rejecto starship.
	 * @param midX
	 *            zero-based column of the midpoint of the object.
	 * @param midY
	 *            zero-based row of the midpoint of the object.
	 * @param confidence
	 *            the fraction of matching pixels between the scope and the
	 *            object's ideal shape (between <code>0.0f</code> - if none of
	 *            the "on" pixels matched - and <code>1.0f</code> - if all of
	 *            the "on" pixels matched).
	 */
	...
	public String toString() {
		return MessageFormat.format("Possible"
				+ " {0,choice,0#slime torpedo|1#Rejecto starship}"
				+ " at location ({1,number,integer}; {2,number,integer})"
				+ " with confidence {3,number,0.0%}.", type, midX, midY,
				confidence);
	}

	/**
	 * Two candidate targets are equal if they are of the same type and location
	 * (confidence is not considered).
	 */
	@Override
	...
	/**
	 * Hash value generated from the type and location (but is unaffected by
	 * confidence). This is to make it consistent with the definition of
	 * <code>equals</code>.
	 */
	@Override
	public int hashCode() {
	...
	/**
	 * Return a comparator that orders by descending confidence first, then by
	 * increasing midX, midY and type.
	 */
	...
	/**
	 * Comparator class returned by the <code>confidenceOrder</code> method.
	 * This class does not need to be visible outside of the parent class,
	 * since it is just the implementation class returned by the
	 * <code>CandidateTarget.confidenceOrder()</code> method.
	 */
	private static final class ConfidenceOrder implements
			Comparator<CandidateTarget> {

...
public final class Analyzer {
  /**
   * Given some scope data, and two thresholds - the minimum confidence level, and the maximum
   * number of candidates - return a list of candidate targets, sorted in descending order of
   * confidence.
   */
  public static List<CandidateTarget> findCandidateTargets(
      Bitmap scope, float limitConfidence, int limitCount) throws IOException {
    ...
    float[][][] confidences = new float[][][] {torpedoConfidence, starshipConfidence};
    List<CandidateTarget> candidates =
        confidencesToCandidates(targets, confidences, limitConfidence);
    ...
    List<CandidateTarget> ret =
        new ArrayList<CandidateTarget>(Math.min(candidates.size(), limitCount));
  ...
  /**
   * Calculate confidence as the fraction of pixels within the target's shape that are "on" in the
   * scope image, assuming the target is positioned at each possible location in the scope image. We
   * only check positions where the target would be located entirely within the scope image (since
   * if the target is only partially visible, we're not going to be sure anyway).
   */
  private static float[][] calculateConfidence(Bitmap target, Bitmap scope) {
    ...
    for (int i = 0; i < 1 + scopeHeight - targetHeight; ++i) {
      for (int j = 0; j < 1 + scopeWidth - targetWidth; ++j) {
        ...
        confidence[i][j] = (float) targetWeight / (float) targetCardinality;
      }
    }
    return confidence;
  }
  ...
  /**
   * Convert an array of confidence arrays into a list of candidate targets by ignoring any elements
   * in the confidence array that have less than the given threshold.
   *
   * <p>The target objects are needed only for their midpoint information - to convert the array
   * indices within the corresponding confidence array (which refer to the top-left of the target's
   * bitmap) into target locations which refer to the midpoint of the target's bitmap.
   */
      ...
      for (int j = 0; j < width; ++j) {
        for (int k = 0; k < ntargets; ++k) {
          if (confidences[k][i][j] >= limitConfidence) {
            ret.add(
                new CandidateTarget(
                    k, j + targets[k].midX(), i + targets[k].midY(), confidences[k][i][j]));
          }
        }
      }
    }

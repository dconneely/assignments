...
final class TestDataGenerator {
  ...
  public static void main(String[] args) throws IOException {
    ...
    Bitmap bitmap =
        generateScope(
            WIDTH, HEIGHT, new Random(), NUM_STARSHIPS, NUM_TORPEDOES, PROB_FLIP_ON, PROB_FLIP_OFF);
  ...
  /**
   * Create a <code>Bitmap</code> of the given width and height. It will have had the specified
   * number of starships and torpedoes placed within it, and then noise applied to the resultant
   * pixels (probabilities for an "off" pixel outside of a target flipping "on" and for an "on"
   * pixel inside a target's shape flipping "off" can be given separately).
   *
   * <p>For testing, we can use a seeded <code>Random</code> random number generator to produce
   * predictable scopes. Note also that the target locations are picked before the noise is applied,
   * so the pseudo-random locations of the targets will not change if a <code>Random</code> instance
   * with the same seeding is used, even if the <code>probFlipOn</code> and <code>probFlipOff</code>
   * probabilities are varied.
   */
  static Bitmap generateScope(
      int width,
      int height,
      Random rand,
      int numStarships,
  ...
  private static void placeTarget(Bitmap shape, boolean[][] data, int leftX, int topY) {
